...
mask = f.replace('+', " ")    # Tạo một mảng đệm khi xóa dấu cộng
arr = mask.replace('x^', " ") # Thay x^ thành khoảng trắng để mảng còn [1 2 3 4 ..... 206]
arr = arr.split()             # Tách chuỗi trên thành mảng ['1','2',....,'206']
Arr_Binary = []               # Tạo mảng để chứa giá trị binary
for i in range(208):
    Arr_Binary.append(0)
for c in arr:
    Arr_Binary[int(c)]=1
Arr_Binary[0]=1               # Vì mảng bắt đầu bằng 1 nên ta phải chuyển nó thành 0
Arr_Binary[1]=0
j=207
while (j>=0):
    print(Arr_Binary[j],end='')
    if j % 8 == 0:
        print(" ")
    j=j-1
